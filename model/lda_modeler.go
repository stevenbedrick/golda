...
func NewModeler(ntopics int, alpha float64, beta float64, iterations int, repetitions int, seed int, parallel bool) LdaModeler {
	to_ret := new(LdaModeler)
	to_ret.ntopics = ntopics
	to_ret.alpha = alpha
	to_ret.beta = beta
	to_ret.iterations = iterations
	to_ret.repetitions = repetitions
	to_ret.seed = seed
	to_ret.parallel = parallel
	return *to_ret
}
...
func (l *LdaModeler) Model(tokenss [][]string) {
	...
	if l.parallel {
		...
		theta_hat_ds, phi_hats, log_likelihood, perplexity := l.model_one_pass(tokenss, l.seed, initial_topicss)
	
		theta_hat_ds_samples[rep_num] = theta_hat_ds
		phi_hats_samples[rep_num] = phi_hats
		log_likelihood_samples[rep_num] = log_likelihood
		perplexity_samples[rep_num] = perplexity
		...
		for rep_num := 0; rep_num < l.repetitions; rep_num++ {
			fmt.Printf("Starting repetition %d of %d.\n", rep_num + 1, l.repetitions)
			theta_hat_ds, phi_hats, log_likelihood, perplexity := l.model_one_pass(tokenss, l.seed, initial_topicss)
	...
	for doc_idx := 0; doc_idx < len(theta_hat_ds_samples[0]); doc_idx++ {
		// for each doc:
		// get the appropriate theta_hat_di for this document from each repetition
...
func resample(r float64, values []float64, values_sum float64) int {
	r *= values_sum
	total := 0.0
...
func (l *LdaModeler) model_one_pass(tokenss [][]string, seed int, initial_topicss [][]int) ([][]float64, []map[string]float64, float64, float64) {
	ntopics := l.ntopics
	...
	// randomizer.Seed(len(tokenss + seed))
	...
	for i, tokens := range(tokenss) { // for each document
		...
		for j, token := range(tokens) { // for each 
			topic := topics[j]
		...
		document_topics_counts = append(document_topics_counts, counts)
	}
	// total_tokens_count := util.SumInt(document_tokens_counts)
	all_keys := [][]string{}
	for i := range(range_ntopics) {
		all_keys = append(all_keys, util.KeysFromMap(topic_words_counts[i]))
	}
	...
	// maxT := ntopics - 1
	uniform_random_func := rand.Float64 // seems to be equivalent to Python's random()- uniform dist between [0.0, 1.0]
	...
	for iteration := 0; iteration < l.iterations; iteration++ {
		...
		for t_idx, tokens := range(tokenss) { // for each document
			...
			for token_index, token := range(tokens) { // for each token
				...
				new_topic := resample(uniform_random_func(), proportional_probabilities, total_proportional_probabilities)
		...
		fmt.Printf("LDA - iteration %d resulted in %d changes.\n", iteration, change_count)
	} // for iterations
	...
	for t := 0; t < ntopics; t++ { // for each topic
		...
		for token, _ := range(topic_words_counts[t]) { // for each token
			dx[token] = (float64(topic_words_counts[t][token]) + beta) / (float64(total_topic_counts[t]) + betaW)
		}
		phi_hats = append(phi_hats, dx)
	}
	...
	fmt.Printf("LDA - log-likelihood of data given model: %0.8e\n", log_likelihood)
	...
	for doc_idx, tokens := range(tokenss) { // each document
		...
		// fmt.Printf("theta_hat_d at %d: %#v\n", doc_idx, theta_hat_d)
		for _, w := range(tokens) { // each token
			temp_phi_hat_theta_hat := []float64{}
			for _, z := range(range_ntopics) { // each topic
				temp_phi_hat_theta_hat = append(temp_phi_hat_theta_hat, phi_hats[z][w] * theta_hat_d[z])
			}
	...
	fmt.Printf("LDA - mean sample token perplexity of data given model: %0.4f\n", perplexity)
	
	// save final snapshot of token-topic assignment snapshot
	l.last_model_token_topic_sample_assignments = topicss
	
	// return results sample
	return theta_hat_ds, phi_hats, log_likelihood, perplexity
